...
public class Nametags extends Module {
    ...
    public void setup() {
        ...
        this.scaling = registerDouble("Size", "Size", 0.3f, 0.1f, 20.0f);
        ...
        this.entityID = registerBoolean("ID", "ID", false);
        this.sneak = registerBoolean("SneakColor","SneakColor", false);
        this.heldStackName = registerBoolean("StackName", "StackName", false);
        this.whiter = registerBoolean("White", "White", false);
        ...
        outline = registerBoolean("Outline", "Outline", false);
        oWidth = registerDouble("OWidth", "OWidth", 1.5, 0.0, 3.0);
    ...
    private void renderNameTag(final EntityPlayer player, final double x, final double y, final double z, final float delta) {
        ...
        GlStateManager.enableBlend();
        if (outline.getValue()) {
            int color = ColorHolder.toHex(red.getValue(), green.getValue(), blue.getValue());
            drawBorderedRectReliant((float) (-width - 1), (float) (-mc.fontRenderer.FONT_HEIGHT), (float) (width + 2), 1.0f, (float)oWidth.getValue(), 1426064384, color);
        } else {
            drawBorderedRectReliant((float) (-width - 1), (float) (-mc.fontRenderer.FONT_HEIGHT), (float) (width + 2), 1.0f, 1.8f, 1426064384, 855638016);
        }
        //RenderUtil.drawRect((float)(-width - 2), (float)(-(FontUtils.getFontHeight(ModuleManager.isModuleEnabled("CustomFont")) + 1)), width + 2.0f, 1.5f, 1426063360);
        ...
        FontUtils.drawStringWithShadow(false, displayTag, (int)(-width), (int)(-(FontUtils.getFontHeight(false) - 1)), this.getDisplayColour(player));
    ...
    private void renderEnchantmentText(final ItemStack stack, final int x, final int y) {
        ...
        if (stack.getItem() == Items.GOLDEN_APPLE && stack.hasEffect()) {
            FontUtils.drawStringWithShadow(false, "god", (int)(x * 2), (int)enchantmentY, -3977919);
            enchantmentY -= 8;
        }
        ...
        for (int index = 0; index < enchants.tagCount(); ++index) {
            ...
            if (enc != null) {
                ...
                encName += level;
                FontUtils.drawStringWithShadow(false, encName, (int)(x * 2), (int)enchantmentY, -1);
                enchantmentY -= 8;
            }
        }
        if (DamageUtils.hasDurability(stack)) {
            ...
            FontUtils.drawStringWithShadow(false, color + percent + "%", (int)(x * 2), (int)enchantmentY, -1);
        }
    }
