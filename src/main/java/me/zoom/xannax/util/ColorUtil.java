...
/**
 * Idiot original dev skidded this from kami
 * <p>
...
public class ColorUtil {
    ...
    public static int genRainbow() {
        ...
        return HUEtoRGB(hue, 1, 1);
    }

    public static int genRainbow(float speed, int saturation, int brightness) {
        float[] hue = new float[]{(float) (System.currentTimeMillis() % (360f * speed)) / (360f * speed)};
        return HUEtoRGB(hue, saturation, brightness);
    }

    private static int HUEtoRGB(float[] hue, int saturation, int brightness) {
        int rgb = Color.HSBtoRGB(hue[0], saturation, brightness);
        int red = rgb >> 16 & 255;
        int green = rgb >> 8 & 255;
        int blue = rgb & 255;
